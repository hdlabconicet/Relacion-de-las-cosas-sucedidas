...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- variables globales -->
    <xsl:variable name="path_indice" select="'01-indice-nuevo.html'"/>
    ...
    <xsl:template match="/">
        ...
        <xsl:result-document method="text" encoding="utf-8"
            ...
            <xsl:text>&lt;div class="col-1 sm-width-full border-top-thin"&gt;&lt;/div&gt;&#x0A;</xsl:text>
    ...
    <xsl:template match="tei:div">
        <!-- títulos de las secciones con dropdown -->
        ...
        <xsl:text>&lt;h6&gt;Página </xsl:text><xsl:value-of select="replace(replace(@facs, '\.png', ''), 'pg_0+','')"/><xsl:text>&lt;/h6&gt;&lt;span class="link-index"&gt;</xsl:text>
        <xsl:value-of select="substring( tei:p[1], 1, 80)"/>
...
&lt;/svg&gt;&lt;/span&gt;&#x0A;</xsl:text>
        <xsl:text>&lt;/a&gt;&#x0A;</xsl:text>
        <!-- link a cada página -->
        <!-- xsl:text>&lt;h6&gt;Página 1 &lt;/h6&gt;&lt;span class="link-index"&gt;S. C. C. M. No he avisado antes a V. M. porque no he tenido oportunidad, mayormente... &lt;svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-arrow-right" viewBox="0 0 16 16"&gt;
  &lt;path fill-rule="evenodd" d="M1 8a.5.5 0 0 1 .5-.5h11.793l-3.147-3.146a.5.5 0 0 1 .708-.708l4 4a.5.5 0 0 1 0 .708l-4 4a.5.5 0 0 1-.708-.708L13.293 8.5H1.5A.5.5 0 0 1 1 8z"/&gt;
&lt;/svg&gt;&lt;/span&gt;&#x0A;-->
...
&lt;/svg&gt;&lt;/span&gt;&#x0A;</xsl:text>
        ...
        <xsl:text>&lt;/div&gt;&#x0A;&#x0A;</xsl:text><!-- fin div colapsable -->        
        <xsl:text>&lt;/div&gt;&#x0A;&#x0A;</xsl:text><!-- fin seccion -->        
    </xsl:template>
